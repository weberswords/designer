...
7. **Content is the interface.** Clear, concrete UX copy in the project's voice beats clever. Sentence case reads faster. Every button says exactly what happens when pressed — the label is a promise, never "OK"/"Submit". When a project intentionally sets its UI chrome and brand names in lowercase, that convention wins over the sentence-case default, and `detect-design-system` should catch and honor it.
